...
def _save_subscription_to_db(sub_data: dict) -> bool:
    ...
    try:
        ...
        print(f"[auth] Failed to save subscription to DB: {exc}")
        return False
...
def _load_subscription_from_db(subscription_id: str) -> dict | None:
    ...
    try:
        ...
        print(f"[auth] Failed to load subscription from DB: {exc}")
    return None
...
def _load_subscription_by_email_from_db(email: str) -> dict | None:
    ...
    try:
        ...
        print(f"[auth] Failed to look up subscription by email: {exc}")
    return None
