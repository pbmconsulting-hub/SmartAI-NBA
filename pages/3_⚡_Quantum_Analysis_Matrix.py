...
if run_analysis:
    ...
    _analysis_errors = 0  # Track props that failed analysis
    for prop_index, prop in enumerate(props_to_analyze):
        ...
        # ── Per-prop safety net — catch ANY error so one bad prop never
        #    crashes the entire analysis loop.  On failure we append a
        #    minimal fallback result and continue to the next prop. ──
        ...
        try:
            ...
            simulation_output = None  # Sentinel — set by the dispatch branches below
            _sim_kwargs = dict(
            ...
            # ── Simulation safety net — if the dispatch crashed, provide safe defaults ──
            if simulation_output is None:
                _logger.warning(
                    "Simulation dispatch returned None for %s (%s) — using fallback defaults.",
                    player_name, stat_type,
                )
                simulation_output = {
                    "probability_over": 0.5,
                    "simulated_mean": projected_stat,
                    "simulated_std": stat_std,
                    "percentile_10": max(0, projected_stat - 2 * stat_std),
                    "percentile_50": projected_stat,
                    "percentile_90": projected_stat + 2 * stat_std,
                    "adjusted_projection": projected_stat,
                    "simulated_results": [],
                }
            ...
            # ── Fallback: create a minimal error result so this prop isn't silently dropped ──
            _analysis_errors += 1
            _logger.warning(
                "Prop analysis failed for %s (%s): %s",
                prop.get("player_name", "?"), prop.get("stat_type", "?"), _prop_err,
            )
            analysis_results_list.append({
                ...
                "tier": "Bronze", "tier_emoji": "🥉",
                "direction": "SKIP",
                "recommendation": f"ANALYSIS ERROR — {prop.get('player_name', 'Unknown')}",
                "forces": {"over_forces": [], "under_forces": []},
                "should_avoid": True,
                "avoid_reasons": [f"Analysis error: {_prop_err}"],
                "histogram": [], "score_breakdown": {},
                "line_vs_avg_pct": 0, "recent_form_results": [],
                "player_matched": False, "explanation": None,
                "line_sharpness_force": None, "line_sharpness_penalty": 0.0,
                ...
                "player_is_out": False,
                "player_status": "Error",
                "player_status_note": str(_prop_err),
                "player_id": "",
                "_analysis_error": str(_prop_err),
            })
            continue

    if _analysis_errors > 0:
        st.warning(
            f"⚠️ **{_analysis_errors}** prop(s) encountered errors during analysis "
            f"and were assigned fallback results. Check logs for details."
        )
