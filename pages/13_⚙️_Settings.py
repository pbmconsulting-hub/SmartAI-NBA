...
with st.expander("📖 How to Use This Page", expanded=False):
    ...
    **API Keys**
    ...
    - Keys are stored in your session only and never sent to our servers
...
# ============================================================
# SECTION: API Keys
# Configure API keys needed for live data and prop fetching.
...
st.markdown(get_education_box_html(
    ...
    Keys are stored only in this browser session. You can also place them in
    <code>.streamlit/secrets.toml</code> for automatic loading.
    """
), unsafe_allow_html=True)
...
with st.expander("🔑 Configure The Odds API Key (Props from all sportsbooks)", expanded=False):
    ...
    with col_save_key:
        if st.button("💾 Save Odds API Key", use_container_width=True):
            ...
            st.rerun()
    with col_clear_key:
        if st.button("🗑️ Clear Odds API Key", use_container_width=True):
...
with st.expander("🔑 Configure ClearSports API Key (Games, Stats, Injuries, Rosters)", expanded=False):
    ...
    with cs_col1:
        if st.button("💾 Save ClearSports Key", use_container_width=True):
            ...
            st.rerun()
    with cs_col2:
        if st.button("🗑️ Clear ClearSports Key", use_container_width=True):
