...
class TestEnrichWithApiNbaOdds(unittest.TestCase):
    ...
    @patch("data.clearsports_client.fetch_game_odds", side_effect=Exception("API error"))
...
class TestEnrichWithPredictions(unittest.TestCase):
    ...
    @patch("data.clearsports_client.fetch_predictions", return_value=SAMPLE_CS_PREDICTIONS)
    ...
    @patch("data.clearsports_client.fetch_predictions", return_value=[])
    ...
    @patch("data.clearsports_client.fetch_predictions", side_effect=Exception("API down"))
...
# ── Section 3: fetch_todays_games calls new enrichment ───────────────────────

class TestFetchTodaysGamesNewEnrichment(unittest.TestCase):
    """Verify fetch_todays_games calls the new enrichment functions."""

    def setUp(self):
        from data.live_data_fetcher import fetch_todays_games
        self.fn = fetch_todays_games
    ...
    def test_calls_clearsports_odds_enrichment(
        ...
        """fetch_todays_games should call _enrich_games_with_clearsports_odds."""
    ...
    def test_calls_predictions_enrichment(
        ...
        """fetch_todays_games should call _enrich_games_with_predictions."""
        mock_cs.return_value = SAMPLE_GAMES
        self.fn()
...
class TestFetchAllTodaysDataStoresOdds(unittest.TestCase):
    ...
    @patch("data.live_data_fetcher.refresh_historical_data_for_tonight", return_value={})
    @patch("data.live_data_fetcher._record_odds_api_snapshots")
    @patch("data.live_data_fetcher.fetch_team_stats", return_value=True)
    @patch("data.live_data_fetcher.fetch_todays_players_only", return_value=True)
    @patch("data.live_data_fetcher.fetch_todays_games")
    @patch("data.clearsports_client.fetch_predictions", return_value=SAMPLE_CS_PREDICTIONS)
    @patch("data.clearsports_client.fetch_game_odds", return_value=SAMPLE_CS_ODDS)
    def test_stores_cs_odds_in_results(
        ...
        """fetch_all_todays_data should include cs_game_odds in results."""
        mock_games.return_value = SAMPLE_GAMES
        from data.live_data_fetcher import fetch_all_todays_data
        result = fetch_all_todays_data()
    ...
    def test_stores_cs_predictions_in_results(
        ...
        """fetch_all_todays_data should include cs_predictions in results."""
        mock_games.return_value = SAMPLE_GAMES
        from data.live_data_fetcher import fetch_all_todays_data
        result = fetch_all_todays_data()
...
class TestOddsWiringSourceLevel(unittest.TestCase):
    """Source-level checks that wiring code exists in live_data_fetcher.py."""

    def setUp(self):
        import pathlib
        self.src = (
            pathlib.Path(__file__).parent.parent / "data" / "live_data_fetcher.py"
    ...
    def test_historical_refresher_uses_cs_player_stats(self):
        """refresh_historical_data_for_tonight must reference fetch_nba_player_stats."""
        idx = self.src.find("def refresh_historical_data_for_tonight(")
        self.assertGreater(idx, 0)
        snippet = self.src[idx:idx + 5000]
        self.assertIn("fetch_nba_player_stats", snippet)

    def test_historical_refresher_uses_recent_scores(self):
        """refresh_historical_data_for_tonight must reference fetch_recent_scores."""
        idx = self.src.find("def refresh_historical_data_for_tonight(")
        self.assertGreater(idx, 0)
    ...
    def test_fetch_all_stores_cs_predictions(self):
        """fetch_all_todays_data must reference fetch_predictions."""
        idx = self.src.find("def fetch_all_todays_data(")
        self.assertGreater(idx, 0)
        snippet = self.src[idx:idx + 8000]
