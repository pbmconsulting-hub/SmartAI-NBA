...
class TestBuildGamesFromOddsApi(unittest.TestCase):
    ...
    def setUp(self):
        from data.live_data_fetcher import _build_games_from_odds_api
    ...
    @patch("data.odds_api_client.fetch_game_odds", return_value=[])
    def test_returns_empty_when_no_odds(self, mock_odds):
        """Should return empty list when Odds API returns no games."""
        games = self.fn()
        self.assertEqual(games, [])

    @patch("data.odds_api_client.fetch_game_odds", side_effect=Exception("API Error"))
...
class TestFetchTodaysGamesFallback(unittest.TestCase):
    ...
    @patch("data.live_data_fetcher._build_games_from_odds_api")
    @patch("data.clearsports_client.fetch_games_today", return_value=[])
    ...
    @patch("data.clearsports_client.fetch_games_today", side_effect=Exception("Network error"))
