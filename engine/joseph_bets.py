...
def joseph_get_track_record() -> dict:
    ...
    try:
        ...
        roi_estimate = ((wins * 0.8 - losses) / max(total, 1)) * 100
...
def joseph_get_accuracy_by_verdict() -> dict:
    """
    Return Joseph's accuracy grouped by verdict tier (SMASH / LEAN).

    Returns:
        dict: ``{"SMASH": {total, wins, pct}, "LEAN": {total, wins, pct}}``
    """
    try:
        all_bets = load_all_bets(limit=2000)
        joseph_bets = [b for b in all_bets if _is_joseph_bet(b)]
